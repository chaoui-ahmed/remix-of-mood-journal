...
interface EntryCardProps {
  entry: {
    ...
    mood_score: number;
    hashtags: string[];
    // On accepte les deux cas pour éviter les bugs
    date?: string;       
    created_at?: string; 
  };
  onClick?: () => void;
...
export function EntryCard({ entry, onClick }: EntryCardProps) {
  // 1. DÉTECTION INTELLIGENTE DE LA DATE
  // On prend 'date' OU 'created_at' (le premier qui existe)
  const rawDate = entry.date || entry.created_at;
  
  // 2. SÉCURISATION TOTALE
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        // On sécurise aussi le score d'humeur
    ...
    >
      <div className="flex items-start justify-between gap-4 mb-3">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>
            {/* Si la date est valide, on l'affiche. Sinon, texte par défaut. */}
            {isDateSafe 
      ...
      {entry.hashtags && entry.hashtags.length > 0 && (
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {entry.hashtags.map((tag) => (
            <span key={tag} className="text-xs font-medium bg-secondary px-2 py-0.5 border border-border">
              {tag}
            </span>
          ))}
        </div>
